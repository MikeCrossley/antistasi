...
_check1 = false;
{_enemigo = _x;
{if (((side _enemigo == side_red) or (side _enemigo == side_green)) and (_enemigo distance _x < 250) and (not(captive _enemigo))) exitWith {_check1 = true}} forEach units _grupo;
if (_check1) exitWith {};
} forEach allUnits;

_check2 = false;
{_enemigo = _x;
if (((side _enemigo == side_red) or (side _enemigo == side_green)) and (player distance _enemigo < 250) and (not(captive _enemigo))) exitWith {_check2 = true}} foreach allUnits;
